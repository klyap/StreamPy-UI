    ...
    if len(unfound_comps) > 0:
        for unfound in unfound_comps:
            ...
            try:
                ...
                # ALL STREAMS TUPLE
                '''
                # Add in all of original
                for s in j['stream_names_tuple']:
                    if (s.split('_PORT_')[0] not in unfound_comps) and\
                    s not in new_dict['stream_names_tuple']:

                        new_dict['stream_names_tuple'].append(s)
                        #print 'added s from j[stream_names_tuple]:'
